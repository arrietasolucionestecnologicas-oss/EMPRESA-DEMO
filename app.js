...
async function shareQuote() {
    ...
    if (incDesc && CART.length > 0) {
        ...
        if (firstImgUrl) {
            ...
            try {
                fileToShare = await getFileFromUrlAsync(firstImgUrl, 'cotizacion_tu_empresa');
    ...
    if (hasImage && navigator.canShare) {
        var shareData = {
            title: "Cotización TU_EMPRESA_AQUI",
            text: msg,
            files: [fileToShare]
        };
